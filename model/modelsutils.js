...
class Setting {
  constructor () {
    /** 默认设置 */
    this.defPath = `${_path}/plugins/bxianx-plugin/defSet/`
    this.defSet = {}

    /** 用户设置 */
    this.configPath = `${_path}/plugins/bxianx-plugin/config/`
    this.config = {}

    this.dataPath = `${_path}/plugins/bxianx-plugin/data/`
    this.data = {}
  ...
  getFilePath (app, type) {
    if (type === 'defSet') return `${this.defPath}${app}.yaml`
      ...
      try {
        ...
        logger.error(`不羡仙缺失默认文件[${app}]${error}`)
      }
  ...
  watch (file, app, type = 'defSet') {
    ...
    watcher.on('change', path => {
      ...
      logger.mark(`[不羡仙插件][修改配置文件][${type}][${app}]`)
      if (this[`change_${app}`]) {
        this[`change_${app}`]()
      }
    })
