...
export default async function (path, params, cfg) {
  ...
  let data = {
    ...
    sys: {
      scale: 1,
      copyright: `Created By Yunzai-Bot<span class="version">${Version.yunzai}</span> & 不羡仙-Plugin<span class="version">${Version.ver}</span>`
    },
    quality: 100
  }
  ...
  let img = await puppeteer.screenshot(`${PluginName}/${app}/${tpl}`, data)
  let ret = true
  if (img) {
