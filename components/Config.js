...
if (!Bot.Bxianx_plugin) Bot.Bxianx_plugin = {}
class Config {
  constructor () {
    this.config = {}

    Bot.Bxianx_plugin.config = this.config
  ...
  getYaml (app, name, type) {
    ...
    try {
      this.config[type][key] = YAML.parse(fs.readFileSync(file, 'utf8'))
    } catch (error) {
      logger.error(`[Bxianx插件][${app}][${name}] 格式错误 ${error}`)
      return false
    }
  ...
  getFilePath (app, name, type) {
    ...
    let configPath = `${PluginPath}/${type}/`
    let file = `${configPath}${app}.${name}.yaml`
    try {
      if (!fs.existsSync(file)) {
        let defaultFile = `${configPath}default/${app}.${name}.yaml`
  ...
  watch (file, app, name, type = 'defSet') {
    ...
    watcher.on('change', (path) => {
      ...
      logger.mark(`[Bxianx插件][修改配置文件][${type}][${app}][${name}]`)
