...
import path from 'path'; // 完整导入 path 模块
...
let CONFIG_YAML;
try {
    CONFIG_YAML = YAML.parse(
        await fs.readFile(path.join(PluginPath, 'config', 'config.yaml'), 'utf8')
    );
} catch (error) {
...
async function readMasterQQFromYaml() {
    ...
    // 修正路径以定位到 Yunzai/config/config/other.yaml
    const yamlFilePath = path.join(__dirname, '../../../config/config/other.yaml');
...
async function readSmsZrMasterQQ() {
    const yamlFilePath = path.join(__dirname, '../config/SMS/SMSzr.js');
    const yamlData = await readYamlFile(yamlFilePath);
...
async function readSmsConfigFromYaml() {
    const yamlFilePath = path.join(__dirname, '../config/SMS/SMS.js');
